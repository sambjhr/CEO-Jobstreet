...
import { CANDIDATES, STAGES, REGIONS } from './data';
import { CandidateRow } from './dashboard';

export function Browse({ onNav }) {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="muted" style={{ margin: 0, fontSize: 15 }}>
            <b style={{ color: "var(--text)", fontVariantNumeric: "tabular-nums" }}>{CANDIDATES.length}</b> vetted CEO members in the register. All introductions are member-approved before contact details release.
          </p>
        </div>
      </div>
      ...
      <div className="browse-grid">
        ...
        <aside>
          ...
          <div className="filter-section">
            ...
            <div className="filter-list">
              {STAGES.filter(s => counts.stg[s.id]).map(s => (
                <button
                  key={s.id}
                  className="filter-item"
          ...
          <div className="filter-section">
            ...
            <div className="filter-list">
              {REGIONS.filter(r => counts.reg[r]).map(r => (
                <button
                  key={r}
                  className="filter-item"
        ...
        <div>
          ...
          {activeFilterCount > 0 && (
            <div className="active-filters">
              <span className="mute2" style={{ fontSize: 12, marginRight: 4 }}>{filtered.length} of {CANDIDATES.length}</span>
              {query && (
