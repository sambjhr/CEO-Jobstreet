// invite.jsx — invite flow modal (referral candidate)

function InviteModal({ open, onClose, kind = "invite" }) {
  // kind: "invite" = invite a CEO candidate; "intro" = request intro to existing candidate
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="steps">
            <Step n={1} label="Candidate" on={step >= 0} done={step > 0} />
            <span className="step-line" />
            <Step n={2} label="Sponsorship" on={step >= 1} done={step > 1} />
            <span className="step-line" />
            <Step n={3} label="Send" on={step >= 2} done={step > 2} />
          </div>
...
// Intro request modal — request introduction to an existing candidate
function IntroRequestModal({ open, onClose, candidateId }) {
  const c = window.CANDIDATES.find(x => x.id === candidateId) || window.CANDIDATES[0];
  const [mandate, setMandate] = React.useState(window.MANDATES[0].id);
  const [note, setNote] = React.useState("");
  const [sent, setSent] = React.useState(false);

  React.useEffect(() => { if (open) { setSent(false); setNote(""); } }, [open]);
  if (!open) return null;
          ...
          {sent ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <p className="muted" style={{ fontSize: 14, maxWidth: 360, margin: "0 auto" }}>
                You can track this in <b style={{ color: "var(--text)" }}>Mandates → {window.MANDATES.find(m => m.id === mandate)?.code}</b>. The candidate's decision is binding for this request.
              </p>
            </div>
          ) : (
            <>
              <div className="form-row">
                ...
                <select className="input" value={mandate} onChange={e => setMandate(e.target.value)}>
                  {window.MANDATES.map(m => <option key={m.id} value={m.id}>{m.code} · {m.title}</option>)}
                </select>
              </div>
              <div className="form-row">
...
Object.assign(window, { InviteModal, IntroRequestModal });
