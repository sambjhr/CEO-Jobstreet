// app.jsx — root: state router, theme, tweaks, modal orchestration
...
function App() {
  ...
  // Router: { view, params }
  // views: landing, dashboard, browse, profile, mandate
  const [route, setRoute] = React.useState({ view: "landing", id: null });
  const [signedIn, setSignedIn] = React.useState(false);

  // Modals
  const [inviteOpen, setInviteOpen] = React.useState(false);
  const [introOpen, setIntroOpen] = React.useState(false);
  const [introCandidate, setIntroCandidate] = React.useState(null);
    ...
    <div className="app">
      ...
      <main style={{ flex: 1 }}>
        ...
        {signedIn && route.view === "dashboard" && (
          <Dashboard
            onNav={nav}
            onOpenMandate={(id) => nav("dashboard")}
            onOpenInvite={() => setInviteOpen(true)}
          />
        )}
